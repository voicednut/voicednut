...
class EnhancedWebhookService {
  constructor(twilioClient = null) {
    ...
    this.updateQueue = new Map(); // callSid → { chatId, timer, messages: [], lastSentStatus }
  }

  /**
   * UNIFIED TELEGRAM NOTIFICATION SYSTEM
   * One pipeline for all call types. Uses requires_input flag to conditionally emit messages.
   */

  /**
   * Send header message for a call (called once on initiation)
   */
  async sendTelegramHeader(chatId, callSid, callConfig) {
   ...
   */
  queueTelegramStatusUpdate(chatId, callSid, statusUpdate) {
    ...
    queue.timer = setTimeout(async () => {
      await this.flushTelegramUpdates(callSid);
    }, 500);
  }
   ...
   */
  async flushTelegramUpdates(callSid) {
    const queue = this.updateQueue.get(callSid);
  ...
  /**
   * Status notification methods (queued/debounced)
   * These are safe to call for ANY call type - they check requires_input internally
   */

  async notifyTelegramInitiated(chatId, callSid) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '📤',
      text: 'Call initiated.'
    });
  }

  async notifyTelegramRinging(chatId, callSid) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '🔔',
  ...
  async notifyTelegramAnswered(chatId, callSid, answeredBy = null) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '✅',
  ...
  async notifyTelegramInProgress(chatId, callSid) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '🟢',
  ...
  /**
   * INPUT-SPECIFIC NOTIFICATIONS (checked for requires_input flag)
   * These methods silently skip for non-input calls
   */

  async notifyTelegramInputAwaiting(chatId, callSid, stepLabel, expectedLen) {
  ...
  async notifyTelegramInputReceived(chatId, callSid, maskedDisplay, attempt = 1) {
    // Only emit input messages if call requires input
    const callRecord = await this.db.getCall(callSid);
    if (!callRecord || !callRecord.requires_input) {
      return; // Silent skip for non-input calls
  ...
  async notifyTelegramInputRejected(chatId, callSid, reason, attempt, maxRetries) {
    // Only emit input messages if call requires input
    const callRecord = await this.db.getCall(callSid);
    if (!callRecord || !callRecord.requires_input) {
      return; // Silent skip for non-input calls
  ...
  /**
   * TERMINAL STATUS NOTIFICATIONS
   */

  async notifyTelegramCompleted(chatId, callSid, reason = null) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '🏁',
      text: 'Call has ended.'
    });
  ...
  async notifyTelegramBusy(chatId, callSid) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '🚫',
      text: 'Line is busy.'
    });
  ...
  async notifyTelegramNoAnswer(chatId, callSid) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '⏳',
      text: 'No answer.'
    });
  ...
  async notifyTelegramCanceled(chatId, callSid) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '⚠️',
  ...
  async notifyTelegramFailed(chatId, callSid, reason = null) {
    this.queueTelegramStatusUpdate(chatId, callSid, {
      emoji: '❌',
  ...
  /**
   * Send final outcome message with inline buttons
   */
  async sendTelegramFinalOutcome(chatId, callSid, outcome) {
    const callRecord = await this.db.getCall(callSid);
    if (!callRecord) {
    ...
    try {
      // Inline buttons for call details (only show if capabilities allow)
      const buttons = [];
      if (callRecord.has_transcript) {
        buttons.push({ text: '📝 View Transcript', callback_data: `transcript:${callSid}` });
      }
      if (callRecord.has_recording) {
        buttons.push({ text: '🎧 Recording', callback_data: `recording:${callSid}` });
      }
      buttons.push({ text: '📊 Call Details', callback_data: `details:${callSid}` });
   ...
   */
  async handleTelegramCallbackQuery(chatId, callbackData) {
    ...
    if (action === 'recording') {
      ...
      if (!call.has_recording) {
        return { text: '🎧 Recording not available.' };
      }
      return { text: '🎧 Recording: [Link would be provided here]' };
    }
   ...
   */
  cancelPendingTelegramUpdates(callSid) {
    const queue = this.updateQueue.get(callSid);
  ...
  cleanupCallData(callSid) {
    ...
    this.cancelPendingTelegramUpdates(callSid); // Also clean up telegram queue
  }
